...
def read_secret(secret_name: str, env_fallback: str, default: str) -> str:
    """Read a Docker secret from file, falling back to env var."""
...
DB_USER = read_secret("db_user", "POSTGRES_USER", "admin")
DB_PASS = read_secret("db_password", "POSTGRES_PASSWORD", "password")
DB_HOST = os.getenv("POSTGRES_HOST", "postgres")
DB_PORT = os.getenv("POSTGRES_PORT", "5432")
DB_NAME = read_secret("db_name", "POSTGRES_DB", "myapp")
...
def get_latest_loot(session: Session = Depends(get_session)):
    ...
    if cached_name:
        return {"item_name": cached_name, "source": "cache (lightning fast!)"}
    ...
    return {"item_name": latest.item_name, "source": "database (slow & steady)"}
